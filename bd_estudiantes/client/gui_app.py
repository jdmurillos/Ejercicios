import tkinter as tk
from tkinter import ttk
from tkinter import messagebox
from model.estudiante_dao import crear_tabla, borrar_tabla
from model.estudiante_dao import Estudiante, guardar, listar, editar, eliminar

def barra_menu(root):
    barra_menu = tk.Menu(root)
    root.config(menu=barra_menu,width=300, height=300)

    menu_inicio = tk.Menu(barra_menu, tearoff= 0)
    barra_menu.add_cascade(label='Inicio', menu=menu_inicio)

    menu_inicio.add_command(label='Crear registro', command=crear_tabla)
    menu_inicio.add_command(label='Eliminar registro', command=borrar_tabla)
    menu_inicio.add_command(label='Salir', command= root.destroy)

    barra_menu.add_cascade(label='Consultas')
    barra_menu.add_cascade(label='Configuración')
    barra_menu.add_cascade(label='Ayuda')


class Frame(tk.Frame):
    def __init__(self, root= None):
        super().__init__(root,width = 480, height=320)
        self.root = root
        self.pack()
        self.config(bg='#EADCF6')
        self.id_estudiante = None
        self.campos_nombre()
        self.desabilitar_campos()
        self.tabla_estudiante()

    def campos_nombre(self):

        #labels
        self.label_nombre = tk.Label(self, text='Nombre:')
        self.label_nombre.config(font= ('Arial', 12, 'bold'))
        self.label_nombre.grid(row= 0, column= 0, padx= 10, pady= 10)

        self.label_apellidos = tk.Label(self, text='Apellidos:')
        self.label_apellidos.config(font= ('Arial', 12, 'bold'))
        self.label_apellidos.grid(row= 1, column= 0, padx= 10, pady= 10)

        self.label_noCelular = tk.Label(self, text='No Celular:')
        self.label_noCelular.config(font= ('Arial', 12, 'bold'))
        self.label_noCelular.grid(row= 2, column= 0, padx= 10, pady= 10)

        self.label_nacimiento = tk.Label(self, text='Fecha Nacimiento:')
        self.label_nacimiento.config(font= ('Arial', 12, 'bold'))
        self.label_nacimiento.grid(row= 3, column= 0, padx= 10, pady= 10)

        #Entradas
        self.mi_nombre = tk.StringVar()
        self.entry_nombre = tk.Entry(self, textvariable= self.mi_nombre)
        self.entry_nombre.config(width = 50)
        self.entry_nombre.grid(row= 0, column= 1, padx=10, pady=10, columnspan= 2)

        self.mi_apellidos = tk.StringVar()
        self.entry_apellidos = tk.Entry(self, textvariable=self.mi_apellidos)
        self.entry_apellidos.config(width = 50)
        self.entry_apellidos.grid(row= 1, column= 1, padx=10, pady=10,columnspan= 2)

        self.mi_noCelular = tk.StringVar()
        self.entry_noCelular = tk.Entry(self, textvariable=self.mi_noCelular)
        self.entry_noCelular.config(width = 50)
        self.entry_noCelular.grid(row= 2, column= 1, padx=10, pady=10, columnspan= 2)

        self.mi_nacimiento = tk.StringVar()
        self.entry_nacimiento = tk.Entry(self, textvariable=self.mi_nacimiento)
        self.entry_nacimiento.config(width = 50)
        self.entry_nacimiento.grid(row= 3, column= 1, padx=10, pady=10,columnspan= 2)

        #Botones
        self.boton_nuevo = tk.Button(self, text = "Nuevo", command = self.habilitar_campos)
        self.boton_nuevo.config(width= 20, font=('Arial',12,'bold'), fg='#DAD5D6', bg = '#158645', cursor ='hand2', activebackground='#35BB6F')
        self.boton_nuevo.grid(row = 4, column= 0, padx=10, pady=10)

        self.boton_guardar = tk.Button(self, text = "Guardar", command= self.guardar_datos)
        self.boton_guardar.config(width= 20, font=('Arial',12,'bold'), fg='#FDFBFF', bg = '#1658A2', cursor ='hand2', activebackground='#3586DF')
        self.boton_guardar.grid(row = 4, column= 1, padx=10, pady=10)

        self.boton_cancelar = tk.Button(self, text = "Cancelar", command= self.desabilitar_campos)
        self.boton_cancelar.config(width= 20, font=('Arial',12,'bold'), fg='#DAD5D6', bg = '#BD152E', cursor ='hand2', activebackground='#E15370')
        self.boton_cancelar.grid(row = 4, column= 2, padx=10, pady=10)

    def habilitar_campos(self):
        self.mi_nombre.set('')
        self.mi_apellidos.set('')
        self.mi_noCelular.set('')
        self.mi_nacimiento.set('')

        self.entry_nombre.config(state='normal')
        self.entry_apellidos.config(state='normal')
        self.entry_noCelular.config(state='normal')
        self.entry_nacimiento.config(state='normal')

        self.boton_guardar.config(state='normal')
        self.boton_cancelar.config(state='normal')



    def desabilitar_campos(self):
        self.id_estudiante = None
        self.mi_nombre.set('')
        self.mi_apellidos.set('')
        self.mi_noCelular.set('')
        self.mi_nacimiento.set('')


        self.entry_nombre.config(state='disabled')
        self.entry_apellidos.config(state='disabled')
        self.entry_noCelular.config(state='disabled')
        self.entry_nacimiento.config(state='disabled')

        self.boton_guardar.config(state='disabled')
        self.boton_cancelar.config(state='disabled')

    def guardar_datos(self):
        
        estudiante = Estudiante(
            self.mi_nombre.get(),
            self.mi_apellidos.get(),
            self.mi_noCelular.get(),
            self.mi_nacimiento.get(),
        ) 

        if self.id_estudiante == None:
            guardar(estudiante)
        else:
            editar(estudiante,self.id_estudiante)

            
        guardar(estudiante)

        self.desabilitar_campos()
        self.tabla_estudiante()


    #Tabla

    def tabla_estudiante(self):

        #Recuperar la lista creada
        self.lista_estudiantes = listar()
        self.lista_estudiantes.reverse()

        self.tabla = ttk.Treeview(self,
        column = ('nombre','apellidos','noCelular','nacimiento' ))
        self.tabla.grid(row=5, column =0, columnspan= 5, sticky='nse')

        #scrollbar para la tabla:
        self.scroll = ttk.Scrollbar(self, orient = 'vertical' , command = self.tabla.yview)
        self.scroll.grid(row = 5, column= 5, sticky='nse')
        self.tabla.configure(yscrollcommand = self.scroll.set)                  

        self.tabla.heading('#0', text='ID')
        self.tabla.heading('#1', text='Nombre')
        self.tabla.heading('#2', text='Apellido')
        self.tabla.heading('#3', text='No Celular')
        self.tabla.heading('#4', text='Nacimiento')

        #iteración
        for p in self.lista_estudiantes:
            self.tabla.insert('',0, text=p[0], values = (p[1], p[2],p[3],p[4]))

        #Boton Editar

        self.boton_editar = tk.Button(self, text = "Editar", command=self.editar_datos)
        self.boton_editar.config(width= 20, font=('Arial',12,'bold'), fg='#DAD5D6', bg = '#158645', cursor ='hand2', activebackground='#35BB6F')
        self.boton_editar.grid(row = 6, column= 0, padx=10, pady=10)

        #Boton Eliminar

        self.boton_eliminar = tk.Button(self, text = "Eliminar", command= self.eliminar_datos)   
        self.boton_eliminar.config(width= 20, font=('Arial',12,'bold'), fg='#DAD5D6', bg = '#BD152E', cursor ='hand2', activebackground='#E15370')
        self.boton_eliminar.grid(row = 6, column= 1, padx=10, pady=10)

    def editar_datos(self):
        try:
            self.id_estudiante = self.tabla.item(self.tabla.selection())['text']
            self.nombre_estudiante = self.tabla.item(self.tabla.selection())['values'][0]
            self.apellidos_estudiante= self.tabla.item(self.tabla.selection())['values'][1]
            self.no_celular_estudiante= self.tabla.item(self.tabla.selection())['values'][2]
            self.fecha_nacimiento_estudiante = self.tabla.item(self.tabla.selection())['values'][3]

            self.habilitar_campos()

            self.entry_nombre.insert(0, self.nombre_estudiante)
            self.entry_apellidos.insert(0, self.apellidos_estudiante)
            self.entry_noCelular.insert(0, self.no_celular_estudiante)
            self.entry_nacimiento.insert(0, self.fecha_nacimiento_estudiante)
        
        except:
            titulo = 'Edición de datos'
            mensaje = 'No ha seleccionado ningun registro'
            messagebox.showerror(titulo, mensaje)

    def eliminar_datos(self):
        try:
            self.id_estudiante = self.tabla.item(self.tabla.selection())['text']
            eliminar(self.id_estudiante)

            self.tabla_estudiante()
            self.id_estudiante = None

            

        except:
            titulo = 'Eliminar un Registro'
            mensaje = 'No se ha seleccionado ningún registro'
            messagebox.showerror(titulo, mensaje)


            







        


